// Code generated by ent, DO NOT EDIT.

package ent

import (
	"context"
	"fmt"
	"math"

	"entgo.io/ent/dialect/sql"
	"entgo.io/ent/dialect/sql/sqlgraph"
	"entgo.io/ent/schema/field"
	"github.com/dipper-iot/dipper-engine-server/ent/predicate"
	"github.com/dipper-iot/dipper-engine-server/ent/rulechan"
	"github.com/dipper-iot/dipper-engine-server/ent/rulenode"
)

// RuleNodeQuery is the builder for querying RuleNode entities.
type RuleNodeQuery struct {
	config
	limit      *int
	offset     *int
	unique     *bool
	order      []OrderFunc
	fields     []string
	predicates []predicate.RuleNode
	withChain  *RuleChanQuery
	// intermediate query (i.e. traversal path).
	sql  *sql.Selector
	path func(context.Context) (*sql.Selector, error)
}

// Where adds a new predicate for the RuleNodeQuery builder.
func (rnq *RuleNodeQuery) Where(ps ...predicate.RuleNode) *RuleNodeQuery {
	rnq.predicates = append(rnq.predicates, ps...)
	return rnq
}

// Limit adds a limit step to the query.
func (rnq *RuleNodeQuery) Limit(limit int) *RuleNodeQuery {
	rnq.limit = &limit
	return rnq
}

// Offset adds an offset step to the query.
func (rnq *RuleNodeQuery) Offset(offset int) *RuleNodeQuery {
	rnq.offset = &offset
	return rnq
}

// Unique configures the query builder to filter duplicate records on query.
// By default, unique is set to true, and can be disabled using this method.
func (rnq *RuleNodeQuery) Unique(unique bool) *RuleNodeQuery {
	rnq.unique = &unique
	return rnq
}

// Order adds an order step to the query.
func (rnq *RuleNodeQuery) Order(o ...OrderFunc) *RuleNodeQuery {
	rnq.order = append(rnq.order, o...)
	return rnq
}

// QueryChain chains the current query on the "chain" edge.
func (rnq *RuleNodeQuery) QueryChain() *RuleChanQuery {
	query := &RuleChanQuery{config: rnq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rnq.prepareQuery(ctx); err != nil {
			return nil, err
		}
		selector := rnq.sqlQuery(ctx)
		if err := selector.Err(); err != nil {
			return nil, err
		}
		step := sqlgraph.NewStep(
			sqlgraph.From(rulenode.Table, rulenode.FieldID, selector),
			sqlgraph.To(rulechan.Table, rulechan.FieldID),
			sqlgraph.Edge(sqlgraph.M2O, true, rulenode.ChainTable, rulenode.ChainColumn),
		)
		fromU = sqlgraph.SetNeighbors(rnq.driver.Dialect(), step)
		return fromU, nil
	}
	return query
}

// First returns the first RuleNode entity from the query.
// Returns a *NotFoundError when no RuleNode was found.
func (rnq *RuleNodeQuery) First(ctx context.Context) (*RuleNode, error) {
	nodes, err := rnq.Limit(1).All(ctx)
	if err != nil {
		return nil, err
	}
	if len(nodes) == 0 {
		return nil, &NotFoundError{rulenode.Label}
	}
	return nodes[0], nil
}

// FirstX is like First, but panics if an error occurs.
func (rnq *RuleNodeQuery) FirstX(ctx context.Context) *RuleNode {
	node, err := rnq.First(ctx)
	if err != nil && !IsNotFound(err) {
		panic(err)
	}
	return node
}

// FirstID returns the first RuleNode ID from the query.
// Returns a *NotFoundError when no RuleNode ID was found.
func (rnq *RuleNodeQuery) FirstID(ctx context.Context) (id int, err error) {
	var ids []int
	if ids, err = rnq.Limit(1).IDs(ctx); err != nil {
		return
	}
	if len(ids) == 0 {
		err = &NotFoundError{rulenode.Label}
		return
	}
	return ids[0], nil
}

// FirstIDX is like FirstID, but panics if an error occurs.
func (rnq *RuleNodeQuery) FirstIDX(ctx context.Context) int {
	id, err := rnq.FirstID(ctx)
	if err != nil && !IsNotFound(err) {
		panic(err)
	}
	return id
}

// Only returns a single RuleNode entity found by the query, ensuring it only returns one.
// Returns a *NotSingularError when more than one RuleNode entity is found.
// Returns a *NotFoundError when no RuleNode entities are found.
func (rnq *RuleNodeQuery) Only(ctx context.Context) (*RuleNode, error) {
	nodes, err := rnq.Limit(2).All(ctx)
	if err != nil {
		return nil, err
	}
	switch len(nodes) {
	case 1:
		return nodes[0], nil
	case 0:
		return nil, &NotFoundError{rulenode.Label}
	default:
		return nil, &NotSingularError{rulenode.Label}
	}
}

// OnlyX is like Only, but panics if an error occurs.
func (rnq *RuleNodeQuery) OnlyX(ctx context.Context) *RuleNode {
	node, err := rnq.Only(ctx)
	if err != nil {
		panic(err)
	}
	return node
}

// OnlyID is like Only, but returns the only RuleNode ID in the query.
// Returns a *NotSingularError when more than one RuleNode ID is found.
// Returns a *NotFoundError when no entities are found.
func (rnq *RuleNodeQuery) OnlyID(ctx context.Context) (id int, err error) {
	var ids []int
	if ids, err = rnq.Limit(2).IDs(ctx); err != nil {
		return
	}
	switch len(ids) {
	case 1:
		id = ids[0]
	case 0:
		err = &NotFoundError{rulenode.Label}
	default:
		err = &NotSingularError{rulenode.Label}
	}
	return
}

// OnlyIDX is like OnlyID, but panics if an error occurs.
func (rnq *RuleNodeQuery) OnlyIDX(ctx context.Context) int {
	id, err := rnq.OnlyID(ctx)
	if err != nil {
		panic(err)
	}
	return id
}

// All executes the query and returns a list of RuleNodes.
func (rnq *RuleNodeQuery) All(ctx context.Context) ([]*RuleNode, error) {
	if err := rnq.prepareQuery(ctx); err != nil {
		return nil, err
	}
	return rnq.sqlAll(ctx)
}

// AllX is like All, but panics if an error occurs.
func (rnq *RuleNodeQuery) AllX(ctx context.Context) []*RuleNode {
	nodes, err := rnq.All(ctx)
	if err != nil {
		panic(err)
	}
	return nodes
}

// IDs executes the query and returns a list of RuleNode IDs.
func (rnq *RuleNodeQuery) IDs(ctx context.Context) ([]int, error) {
	var ids []int
	if err := rnq.Select(rulenode.FieldID).Scan(ctx, &ids); err != nil {
		return nil, err
	}
	return ids, nil
}

// IDsX is like IDs, but panics if an error occurs.
func (rnq *RuleNodeQuery) IDsX(ctx context.Context) []int {
	ids, err := rnq.IDs(ctx)
	if err != nil {
		panic(err)
	}
	return ids
}

// Count returns the count of the given query.
func (rnq *RuleNodeQuery) Count(ctx context.Context) (int, error) {
	if err := rnq.prepareQuery(ctx); err != nil {
		return 0, err
	}
	return rnq.sqlCount(ctx)
}

// CountX is like Count, but panics if an error occurs.
func (rnq *RuleNodeQuery) CountX(ctx context.Context) int {
	count, err := rnq.Count(ctx)
	if err != nil {
		panic(err)
	}
	return count
}

// Exist returns true if the query has elements in the graph.
func (rnq *RuleNodeQuery) Exist(ctx context.Context) (bool, error) {
	if err := rnq.prepareQuery(ctx); err != nil {
		return false, err
	}
	return rnq.sqlExist(ctx)
}

// ExistX is like Exist, but panics if an error occurs.
func (rnq *RuleNodeQuery) ExistX(ctx context.Context) bool {
	exist, err := rnq.Exist(ctx)
	if err != nil {
		panic(err)
	}
	return exist
}

// Clone returns a duplicate of the RuleNodeQuery builder, including all associated steps. It can be
// used to prepare common query builders and use them differently after the clone is made.
func (rnq *RuleNodeQuery) Clone() *RuleNodeQuery {
	if rnq == nil {
		return nil
	}
	return &RuleNodeQuery{
		config:     rnq.config,
		limit:      rnq.limit,
		offset:     rnq.offset,
		order:      append([]OrderFunc{}, rnq.order...),
		predicates: append([]predicate.RuleNode{}, rnq.predicates...),
		withChain:  rnq.withChain.Clone(),
		// clone intermediate query.
		sql:    rnq.sql.Clone(),
		path:   rnq.path,
		unique: rnq.unique,
	}
}

// WithChain tells the query-builder to eager-load the nodes that are connected to
// the "chain" edge. The optional arguments are used to configure the query builder of the edge.
func (rnq *RuleNodeQuery) WithChain(opts ...func(*RuleChanQuery)) *RuleNodeQuery {
	query := &RuleChanQuery{config: rnq.config}
	for _, opt := range opts {
		opt(query)
	}
	rnq.withChain = query
	return rnq
}

// GroupBy is used to group vertices by one or more fields/columns.
// It is often used with aggregate functions, like: count, max, mean, min, sum.
//
// Example:
//
//	var v []struct {
//		ChainID int `json:"chain_id,omitempty"`
//		Count int `json:"count,omitempty"`
//	}
//
//	client.RuleNode.Query().
//		GroupBy(rulenode.FieldChainID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
func (rnq *RuleNodeQuery) GroupBy(field string, fields ...string) *RuleNodeGroupBy {
	grbuild := &RuleNodeGroupBy{config: rnq.config}
	grbuild.fields = append([]string{field}, fields...)
	grbuild.path = func(ctx context.Context) (prev *sql.Selector, err error) {
		if err := rnq.prepareQuery(ctx); err != nil {
			return nil, err
		}
		return rnq.sqlQuery(ctx), nil
	}
	grbuild.label = rulenode.Label
	grbuild.flds, grbuild.scan = &grbuild.fields, grbuild.Scan
	return grbuild
}

// Select allows the selection one or more fields/columns for the given query,
// instead of selecting all fields in the entity.
//
// Example:
//
//	var v []struct {
//		ChainID int `json:"chain_id,omitempty"`
//	}
//
//	client.RuleNode.Query().
//		Select(rulenode.FieldChainID).
//		Scan(ctx, &v)
func (rnq *RuleNodeQuery) Select(fields ...string) *RuleNodeSelect {
	rnq.fields = append(rnq.fields, fields...)
	selbuild := &RuleNodeSelect{RuleNodeQuery: rnq}
	selbuild.label = rulenode.Label
	selbuild.flds, selbuild.scan = &rnq.fields, selbuild.Scan
	return selbuild
}

func (rnq *RuleNodeQuery) prepareQuery(ctx context.Context) error {
	for _, f := range rnq.fields {
		if !rulenode.ValidColumn(f) {
			return &ValidationError{Name: f, err: fmt.Errorf("ent: invalid field %q for query", f)}
		}
	}
	if rnq.path != nil {
		prev, err := rnq.path(ctx)
		if err != nil {
			return err
		}
		rnq.sql = prev
	}
	return nil
}

func (rnq *RuleNodeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*RuleNode, error) {
	var (
		nodes       = []*RuleNode{}
		_spec       = rnq.querySpec()
		loadedTypes = [1]bool{
			rnq.withChain != nil,
		}
	)
	_spec.ScanValues = func(columns []string) ([]any, error) {
		return (*RuleNode).scanValues(nil, columns)
	}
	_spec.Assign = func(columns []string, values []any) error {
		node := &RuleNode{config: rnq.config}
		nodes = append(nodes, node)
		node.Edges.loadedTypes = loadedTypes
		return node.assignValues(columns, values)
	}
	for i := range hooks {
		hooks[i](ctx, _spec)
	}
	if err := sqlgraph.QueryNodes(ctx, rnq.driver, _spec); err != nil {
		return nil, err
	}
	if len(nodes) == 0 {
		return nodes, nil
	}
	if query := rnq.withChain; query != nil {
		if err := rnq.loadChain(ctx, query, nodes, nil,
			func(n *RuleNode, e *RuleChan) { n.Edges.Chain = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
}

func (rnq *RuleNodeQuery) loadChain(ctx context.Context, query *RuleChanQuery, nodes []*RuleNode, init func(*RuleNode), assign func(*RuleNode, *RuleChan)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*RuleNode)
	for i := range nodes {
		fk := nodes[i].ChainID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	}
	query.Where(rulechan.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "chain_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
	}
	return nil
}

func (rnq *RuleNodeQuery) sqlCount(ctx context.Context) (int, error) {
	_spec := rnq.querySpec()
	_spec.Node.Columns = rnq.fields
	if len(rnq.fields) > 0 {
		_spec.Unique = rnq.unique != nil && *rnq.unique
	}
	return sqlgraph.CountNodes(ctx, rnq.driver, _spec)
}

func (rnq *RuleNodeQuery) sqlExist(ctx context.Context) (bool, error) {
	switch _, err := rnq.FirstID(ctx); {
	case IsNotFound(err):
		return false, nil
	case err != nil:
		return false, fmt.Errorf("ent: check existence: %w", err)
	default:
		return true, nil
	}
}

func (rnq *RuleNodeQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := &sqlgraph.QuerySpec{
		Node: &sqlgraph.NodeSpec{
			Table:   rulenode.Table,
			Columns: rulenode.Columns,
			ID: &sqlgraph.FieldSpec{
				Type:   field.TypeInt,
				Column: rulenode.FieldID,
			},
		},
		From:   rnq.sql,
		Unique: true,
	}
	if unique := rnq.unique; unique != nil {
		_spec.Unique = *unique
	}
	if fields := rnq.fields; len(fields) > 0 {
		_spec.Node.Columns = make([]string, 0, len(fields))
		_spec.Node.Columns = append(_spec.Node.Columns, rulenode.FieldID)
		for i := range fields {
			if fields[i] != rulenode.FieldID {
				_spec.Node.Columns = append(_spec.Node.Columns, fields[i])
			}
		}
	}
	if ps := rnq.predicates; len(ps) > 0 {
		_spec.Predicate = func(selector *sql.Selector) {
			for i := range ps {
				ps[i](selector)
			}
		}
	}
	if limit := rnq.limit; limit != nil {
		_spec.Limit = *limit
	}
	if offset := rnq.offset; offset != nil {
		_spec.Offset = *offset
	}
	if ps := rnq.order; len(ps) > 0 {
		_spec.Order = func(selector *sql.Selector) {
			for i := range ps {
				ps[i](selector)
			}
		}
	}
	return _spec
}

func (rnq *RuleNodeQuery) sqlQuery(ctx context.Context) *sql.Selector {
	builder := sql.Dialect(rnq.driver.Dialect())
	t1 := builder.Table(rulenode.Table)
	columns := rnq.fields
	if len(columns) == 0 {
		columns = rulenode.Columns
	}
	selector := builder.Select(t1.Columns(columns...)...).From(t1)
	if rnq.sql != nil {
		selector = rnq.sql
		selector.Select(selector.Columns(columns...)...)
	}
	if rnq.unique != nil && *rnq.unique {
		selector.Distinct()
	}
	for _, p := range rnq.predicates {
		p(selector)
	}
	for _, p := range rnq.order {
		p(selector)
	}
	if offset := rnq.offset; offset != nil {
		// limit is mandatory for offset clause. We start
		// with default value, and override it below if needed.
		selector.Offset(*offset).Limit(math.MaxInt32)
	}
	if limit := rnq.limit; limit != nil {
		selector.Limit(*limit)
	}
	return selector
}

// RuleNodeGroupBy is the group-by builder for RuleNode entities.
type RuleNodeGroupBy struct {
	config
	selector
	fields []string
	fns    []AggregateFunc
	// intermediate query (i.e. traversal path).
	sql  *sql.Selector
	path func(context.Context) (*sql.Selector, error)
}

// Aggregate adds the given aggregation functions to the group-by query.
func (rngb *RuleNodeGroupBy) Aggregate(fns ...AggregateFunc) *RuleNodeGroupBy {
	rngb.fns = append(rngb.fns, fns...)
	return rngb
}

// Scan applies the group-by query and scans the result into the given value.
func (rngb *RuleNodeGroupBy) Scan(ctx context.Context, v any) error {
	query, err := rngb.path(ctx)
	if err != nil {
		return err
	}
	rngb.sql = query
	return rngb.sqlScan(ctx, v)
}

func (rngb *RuleNodeGroupBy) sqlScan(ctx context.Context, v any) error {
	for _, f := range rngb.fields {
		if !rulenode.ValidColumn(f) {
			return &ValidationError{Name: f, err: fmt.Errorf("invalid field %q for group-by", f)}
		}
	}
	selector := rngb.sqlQuery()
	if err := selector.Err(); err != nil {
		return err
	}
	rows := &sql.Rows{}
	query, args := selector.Query()
	if err := rngb.driver.Query(ctx, query, args, rows); err != nil {
		return err
	}
	defer rows.Close()
	return sql.ScanSlice(rows, v)
}

func (rngb *RuleNodeGroupBy) sqlQuery() *sql.Selector {
	selector := rngb.sql.Select()
	aggregation := make([]string, 0, len(rngb.fns))
	for _, fn := range rngb.fns {
		aggregation = append(aggregation, fn(selector))
	}
	// If no columns were selected in a custom aggregation function, the default
	// selection is the fields used for "group-by", and the aggregation functions.
	if len(selector.SelectedColumns()) == 0 {
		columns := make([]string, 0, len(rngb.fields)+len(rngb.fns))
		for _, f := range rngb.fields {
			columns = append(columns, selector.C(f))
		}
		columns = append(columns, aggregation...)
		selector.Select(columns...)
	}
	return selector.GroupBy(selector.Columns(rngb.fields...)...)
}

// RuleNodeSelect is the builder for selecting fields of RuleNode entities.
type RuleNodeSelect struct {
	*RuleNodeQuery
	selector
	// intermediate query (i.e. traversal path).
	sql *sql.Selector
}

// Scan applies the selector query and scans the result into the given value.
func (rns *RuleNodeSelect) Scan(ctx context.Context, v any) error {
	if err := rns.prepareQuery(ctx); err != nil {
		return err
	}
	rns.sql = rns.RuleNodeQuery.sqlQuery(ctx)
	return rns.sqlScan(ctx, v)
}

func (rns *RuleNodeSelect) sqlScan(ctx context.Context, v any) error {
	rows := &sql.Rows{}
	query, args := rns.sql.Query()
	if err := rns.driver.Query(ctx, query, args, rows); err != nil {
		return err
	}
	defer rows.Close()
	return sql.ScanSlice(rows, v)
}
