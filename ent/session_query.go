...
func (sq *SessionQuery) QueryChain() *RuleChanQuery {
	query := &RuleChanQuery{config: sq.config}
...
func (sq *SessionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Session, error) {
	...
	if query := sq.withChain; query != nil {
		if err := sq.loadChain(ctx, query, nodes, nil,
			func(n *Session, e *RuleChan) { n.Edges.Chain = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *SessionQuery) loadChain(ctx context.Context, query *RuleChanQuery, nodes []*Session, init func(*Session), assign func(*Session, *RuleChan)) error {
